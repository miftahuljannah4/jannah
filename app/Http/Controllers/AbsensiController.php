...
class AbsensiController extends Controller
{
    ...
    public function index()
    {
        ...
        $lokasi_kantor = DB::table('lokasi_kantor')
            ->where('id', 1)
            ->first();
        $jamkerja = DB::table('hari')
        ->join('jam_kerja','hari.id_jam','=','jam_kerja.id_jam')
        ->where('nik',$nik)->where('hari',$namahari)->first();
        //dd($jamkerja)
        return view('absensi.index', compact('check', 'lokasi_kantor', 'jamkerja'));
    }
        ...
        if ($radius > $lokasi_kantor->radius) {
            ...
            if ($check > 0) {
                if ($jam < $jamkerja->jam_pulang) {
                    echo "error|Oops, it's not time to go home yet|out";
                }else{
                    $data_pulang = [
                        'jam_out' => $jam,
                        'foto_out' => $fileName,
                ...
                if ($jam < $jamkerja->awal_jam_masuk) {
                    echo "error|Sorry, it's not time to attendance |in";
                } else if($jam > $jamkerja->akhir_jam_masuk){
                    echo "error|Sorry,you can't make attendance anymore |in";
                } else {
                    $data_masuk = [
                        'nik' => $nik,
    ...
    public function getHistory(Request $request)
    {
        ...
        $history = DB::table('absensis')->leftJoin('jam_kerja','absensis.id_jam','=','jam_kerja.id_jam')
    ...
    public function cekdata(Request $request)
    {
        ...
        $absensi = DB::table('absensis')
            ->select('absensis.*','nik_karyawan', 'nama_lengkap', 'nama_pd','jam_masuk','nama_jam','jam_pulang')
            ->leftJoin('jam_kerja','absensis.id_jam','=','jam_kerja.id_jam')
    ...
    public function printlaporan(Request $request)
    {
        ...
        $absensi = DB::table('absensis')->leftJoin('jam_kerja','absensis.id_jam','=','jam_kerja.id_jam')
            ->where('nik', $nik)
            ->whereRaw('MONTH(tgl_absensi)="' . $bulan . '"')
            ->whereRaw('YEAR(tgl_absensi)="' . $tahun . '"')
            ->orderBy('tgl_absensi')
    ...
    public function printlaporanabsen(Request $request)
    {
        ...
        $printabsensi = DB::table('absensis')
            ->selectRaw(
                'absensis.nik,nik_karyawan,nama_lengkap,jam_masuk,jam_pulang,
        ...
        max(if(day(tgl_absensi) = 31,concat(jam_in,"-",ifnull(jam_out,"00:00:00")),"")) as tgl_31',
            ...
            ->whereRaw('MONTH(tgl_absensi)="' . $bulan . '"')
            ->whereRaw('YEAR(tgl_absensi)="' . $tahun . '"')
            ->groupByRaw('absensis.nik,nik_karyawan,nama_lengkap,jam_masuk,jam_pulang')
            ->get();
